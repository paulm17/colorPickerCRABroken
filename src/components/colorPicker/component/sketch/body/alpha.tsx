...
function Alpha() {
  ...
  const sliderDirectionCSS =
    sliderDirection == "horizontal" ? style.cpAlphaHorizontal : style.cpAlphaVertical
  ...
  const pointerTop = useMemo(() => {
    if (sliderDirection == "vertical") {
      ...
      if (containerRect) {
        let { y, height } = calcXYAlphaCursorPosition(sliderDirection, containerRect, color)
  ...
  const pointerLeft = useMemo(() => {
    if (sliderDirection == "horizontal") {
      ...
      if (containerRect) {
        let { x, width } = calcXYAlphaCursorPosition(sliderDirection, containerRect, color)
